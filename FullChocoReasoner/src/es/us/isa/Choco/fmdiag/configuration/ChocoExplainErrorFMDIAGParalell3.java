...
public class ChocoExplainErrorFMDIAGParalell3 extends ChocoQuestion implements ValidConfigurationErrorsQuestion {
	...
	public ChocoExplainErrorFMDIAGParalell3(int m, int t){
		this.m = m;
	...
	public List<String> fmdiag(List<String> S, List<String> AC) {
		...
		if (S.size() == 0 || !isConsistent(less(AC, S))) {
			...
			diagThreadsFJ dt = new diagThreadsFJ(new ArrayList<String>(), S, AC, numberOfThreads, executorService, 0, 1);
	...
	public class diagThreadsFJ extends RecursiveTask<List<String>>{
		...
		public diagThreadsFJ(List<String> D, List<String> S,List<String> AC,int numberOfSplits, 
				             ExecutorService executorService, int fatherId, int Id){
			this.D=D;
			this.S=S;
			this.AC=AC;
		...
		public List<String> compute(){		
			...
			System.out.println("(" + Id + ", " + fatherId + ")");
		...
		/*outList corresponds to a results list for the threads of the solution*/
			...
			for(List<String> s: splitListToSubLists){
				...
				diagThreadsFJ dt = new diagThreadsFJ(rest, s,less , numberOfSplits, executorService, Id, childId);
				dt.fork();
			...
			diagThreadsFJ dt = new diagThreadsFJ(fullSolution1, s, less , numberOfSplits, executorService, Id, childId);
			dt.fork();
			
			outLists.add(dt.join());						
	    	List<String> fullSolution = plus(outLists);
			
	    	System.out.println("Sol (" + Id + ", " + fatherId + "): " + fullSolution);
